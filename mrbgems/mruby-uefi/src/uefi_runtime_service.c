...
rs_get_all_variable_names(mrb_state *mrb, mrb_value obj)
{
    ...
    while(status == EFI_SUCCESS || status == EFI_BUFFER_TOO_SMALL){
        if (status == EFI_BUFFER_TOO_SMALL){
            ...
            mrb_hash_set(mrb, hash, mrb_symbol_value(mrb_intern(mrb, "guid", sizeof("guid") - 1)),
                         mrb_uefi_guid_set_guid(mrb, &guid));
            mrb_hash_set(mrb, hash, mrb_symbol_value(mrb_intern(mrb, "name", sizeof("name") - 1)),
...
#undef INFO
    ...
    for (i = 0; i < sizeof(info_list)/sizeof(info_list[0]); i++){
        ...
        mrb_const_set(mrb, mrb_obj_value(mrb_ns), mrb_intern(mrb, info->name, sizeof(info->name) - 1),
...
mrb_init_uefi_runtime_service(mrb_state *mrb, struct RClass *mrb_uefi)
{
    ...
    mrb_define_module_function(mrb, mrb_ns, "pointer", rs_pointer, MRB_ARGS_NONE());
    mrb_define_module_function(mrb, mrb_ns, "get_variable", rs_get_variable, MRB_ARGS_REQ(4));
    mrb_define_module_function(mrb, mrb_ns, "set_variable", rs_set_variable, MRB_ARGS_REQ(2));
    mrb_define_module_function(mrb, mrb_ns, "get_all_variable_names", rs_get_all_variable_names, MRB_ARGS_NONE());
    mrb_define_module_function(mrb, mrb_ns, "reset_system", rs_reset_system, MRB_ARGS_ANY());
