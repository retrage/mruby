...
mrb_uefi_status_init_efi_status_code(mrb_state *mrb, struct RClass *st_cls)
{
    ...
    for (i = 0; i < sizeof(efi_status_info_list)/sizeof(efi_status_info_list[0]); i++){
        ...
        mrb_const_set(mrb, st_cls_value, mrb_intern(mrb, info->name, sizeof(info->name) - 1),
...
mrb_init_uefi_status(mrb_state *mrb, struct RClass *mrb_uefi)
{
    ...
    mrb_define_method(mrb, st_cls, "==", mrb_uefi_status_eq, MRB_ARGS_REQ(1));
    mrb_define_method(mrb, st_cls, "error?", mrb_uefi_status_error_p, MRB_ARGS_NONE());
    mrb_define_method(mrb, st_cls, "success?", mrb_uefi_status_success_p, MRB_ARGS_NONE());
    mrb_define_method(mrb, st_cls, "to_s", mrb_uefi_status_to_s, MRB_ARGS_NONE());
    mrb_define_method(mrb, st_cls, "inspect", mrb_uefi_status_inspect, MRB_ARGS_NONE());
    mrb_define_method(mrb, st_cls, "value", mrb_uefi_status_value, MRB_ARGS_NONE());
