...
#define INTERN(mrb, str) mrb_intern(mrb, str, sizeof(str) - 1)
...
{
    mrb_define_method(mrb, mrb->string_class, "pointer", mrb_str_get_pointer, MRB_ARGS_NONE());
}
...
mrb_init_uefi_protocol(mrb_state *mrb, struct RClass *mrb_uefi)
{
    ...
    mrb_define_method(mrb, cls, "call_raw_function", mrb_uefi_protocol_raw_function, MRB_ARGS_REQ(4));
    mrb_define_method(mrb, cls, "get_raw_value", mrb_uefi_protocol_get_raw_value, MRB_ARGS_REQ(2));
    mrb_define_method(mrb, cls, "set_raw_value", mrb_uefi_protocol_set_raw_value, MRB_ARGS_REQ(3));
